...
int main( int argc, char ** argv )
{
...

	{
		...
		if( dtb_file_name )
		{
			...
			fread( ram_image + dtb_ptr, dtblen, 1, f );
			fclose( f );
		}
	}
			...
			case 0b000: //ECALL/EBREAK/WFI
				...
				else
				{
					...
					if( csrno == 0 )
					{
						...
						state->mcause = 8; // 8 = "Environment call from U-mode"
					}
					else
					{
