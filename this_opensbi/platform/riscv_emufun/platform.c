...
#define EMUFUN_DEFAULT_UART_ADDR		0x3F8
...
/*
#define emufun_DEFAULT_PLIC_ADDR		0xfff1100000
#define emufun_DEFAULT_PLIC_NUM_SOURCES	2
#define emufun_DEFAULT_HART_COUNT		3
#define emufun_DEFAULT_CLINT_ADDR		0xfff1020000
#define emufun_DEFAULT_ACLINT_MTIMER_FREQ	1000000
#define emufun_DEFAULT_ACLINT_MSWI_ADDR	\
		(emufun_DEFAULT_CLINT_ADDR + CLINT_MSWI_OFFSET)
#define emufun_DEFAULT_ACLINT_MTIMER_ADDR	\
		(emufun_DEFAULT_CLINT_ADDR + CLINT_MTIMER_OFFSET)
*/
...
static struct plic_data plic = {
	.addr = emufun_DEFAULT_PLIC_ADDR,
	.num_src = emufun_DEFAULT_PLIC_NUM_SOURCES,
};

static struct aclint_mswi_data mswi = {
	.addr = emufun_DEFAULT_ACLINT_MSWI_ADDR,
...
static struct aclint_mtimer_data mtimer = {
	.mtime_freq = emufun_DEFAULT_ACLINT_MTIMER_FREQ,
	.mtime_addr = emufun_DEFAULT_ACLINT_MTIMER_ADDR +
		      ACLINT_DEFAULT_MTIME_OFFSET,
	.mtime_size = ACLINT_DEFAULT_MTIME_SIZE,
	.mtimecmp_addr = emufun_DEFAULT_ACLINT_MTIMER_ADDR +
...
static int emufun_console_init(void)
{
	return uart8250_init(uart.addr,
			     uart.freq,
			     uart.baud,
			     EMUFUN_DEFAULT_UART_REG_SHIFT,
			     EMUFUN_DEFAULT_UART_REG_WIDTH,
			     EMUFUN_DEFAULT_UART_REG_OFFSET);
}

#if 0
static int plic_emufun_warm_irqchip_init(int m_cntx_id, int s_cntx_id)
{
	int ret;
...
/*
 * Initialize the emufun interrupt controller for current HART.
 */
static int emufun_irqchip_init(bool cold_boot)
{
	...
	return plic_emufun_warm_irqchip_init(2 * hartid, 2 * hartid + 1);
}

/*
 * Initialize IPI for current HART.
 */
static int emufun_ipi_init(bool cold_boot)
{
	int ret;
...
/*
 * Initialize emufun timer for current HART.
 */
static int emufun_timer_init(bool cold_boot)
{
	int ret;
